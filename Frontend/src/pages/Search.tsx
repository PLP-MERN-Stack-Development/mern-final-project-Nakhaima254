...
const Search = () => {
  ...
  useEffect(() => {
    ...
    const fetchMedicines = async () => {
      ...
      try {
        const response = await apiClient.getMedicines({ availability: true });
        ...
        const uniqueLocations = [...new Set(formattedMedicines
          .map((m: Medicine) => m.pharmacy?.location)
        ...
        toast({
          ...
          description: error.message || "Failed to load medicine data. Please try again.",
          variant: "destructive",
        });
      } finally {
  ...
  useEffect(() => {
    ...
    if (selectedLocation && selectedLocation !== "all") {
      filtered = filtered.filter((medicine) =>
        medicine.pharmacy?.location === selectedLocation
      );
    }
