...
const Pharmacies = () => {
    ...
    const fetchPharmacies = async () => {
      try {
        ...
        toast({
          ...
          description: error.message || "Failed to load pharmacy data. Please try again.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  const handlePartnerSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For now, just show success message since email service isn't implemented
      // In production, you'd implement a contact/lead submission endpoint
      console.log('Pharmacy partnership application:', formData);
  ...
  const stats = [
    ...
    { label: "Verified Partners", value: pharmacies.filter(p => p.verified).length.toString(), icon: ShieldCheck },
                ...
                <Card className="shadow-card hover:shadow-elevated transition-shadow duration-300">
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      ...
                      {getStatusBadge(pharmacy.verified)}
                    </div>
                  </CardHeader>
